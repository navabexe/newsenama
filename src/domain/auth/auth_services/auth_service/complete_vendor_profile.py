# File: domain/auth/auth_services/auth_service/complete_vendor_profile.py
...
# ------------------------------
# Validation: Business categories
# ------------------------------
async def validate_business_categories(category_ids: List[str]) -> None:
    try:
        ...
        log_error("Category validation failed", extra={"category_ids": category_ids, "error": str(e)}, exc_info=True)
...
async def notify_admin_of_pending_vendor(vendor_id: str, phone: str, business_name: str, client_ip: str):
    try:
        ...
        log_info("Admin notified", extra={"vendor_id": vendor_id, "ip": client_ip})
    except Exception as e:
        log_error("Failed to notify admin", extra={"vendor_id": vendor_id, "error": str(e)}, exc_info=True)
...
async def complete_vendor_profile_service(
    ...
    try:
        ...
        if role != "vendor":
            ...
            raise HTTPException(status_code=403, detail="This endpoint is for vendors only")

        # Fetch vendor
        vendor = await find_one("vendors", {"phone": phone})
        if not vendor or vendor.get("status") not in ["incomplete", "pending"]:
            log_error("Vendor not eligible", extra={"phone": phone, "status": vendor.get("status"), "ip": client_ip})
            raise HTTPException(status_code=400, detail="Profile not eligible")
        ...
        # Update vendor
        modified_count = await update_one("vendors", {"_id": ObjectId(vendor_id)}, update_data)
        if modified_count == 0:
            log_error("No vendor updated", extra={"vendor_id": vendor_id, "ip": client_ip})
            raise HTTPException(status_code=500, detail="Failed to update profile")

        # Get updated vendor
        updated_vendor = await find_one("vendors", {"_id": ObjectId(vendor_id)})
        ...
        is_complete = all(updated_vendor.get(field) for field in required_fields)

        if is_complete and updated_vendor["status"] == "incomplete":
            # Update status to pending
            await update_one("vendors", {"_id": ObjectId(vendor_id)}, {"status": "pending"})
            ...
            scopes = get_scopes_for_role(role, "pending")
        ...
        raise HTTPException(status_code=500, detail="Failed to complete vendor profile")