...
async def login_vendor_service(phone: str, password: str, client_ip: str) -> dict:
    try:
        ...
        if not vendor:
            log_error("Vendor not found", extra={"phone": phone, "ip": client_ip})
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")

        if "password" not in vendor or not vendor["password"]:
            log_error("Vendor has no password set", extra={"phone": phone, "ip": client_ip})
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")

        # Account must be active
        if vendor.get("status") != "active":
            log_error("Vendor account not active", extra={
                "phone": phone,
                "status": vendor.get("status"),
                "ip": client_ip
            })
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Account is {vendor.get('status')}")
        ...
        access_token = generate_access_token(vendor_id, role, session_id, scopes=scopes)
        refresh_token = generate_refresh_token(vendor_id, role, session_id)

        # Store session in Redis
        session_key = f"sessions:{vendor_id}:{session_id}"
        ...
        log_error("Vendor login failed", extra={
            "phone": phone,
            "error": str(e),
            "ip": client_ip
        }, exc_info=True)
